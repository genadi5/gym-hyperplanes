...
def create_coefficient_fraction(sign, coefficient):
    if float(coefficient) == 0:
        return Fraction(0)
    if '.' in coefficient:
        return Fraction(sign + coefficient)
    return Fraction(sign + coefficient + "/1")
...
class Simplex(object):
    ...
    def construct_matrix_from_constraints(self):
        ...
        for i in range(1, len(self.constraints) + 1):
            ...
            for j in range(len(constraint)):
                ...
                if '_' in constraint[j]:
                    ...
                    if constraint[j - 1] is '-':
                        coeff_matrix[i][int(index) - 1] = create_coefficient_fraction("-", coeff[:-1])
                    else:
                        coeff_matrix[i][int(index) - 1] = create_coefficient_fraction("", coeff[:-1])
            ...
            coeff_matrix[i][-1] = create_coefficient_fraction("", constraint[-1])
        ...
        for i in range(len(objective_function_coeffs)):
            if '_' in objective_function_coeffs[i]:
                ...
                if objective_function_coeffs[i - 1] is '-':
                    self.coeff_matrix[0][int(index) - 1] = create_coefficient_fraction("", coeff[:-1])
                else:
                    self.coeff_matrix[0][int(index) - 1] = create_coefficient_fraction("-", coeff[:-1])
